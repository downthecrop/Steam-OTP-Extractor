...
bold "Steam Guard Extractor (Linux)"
info  "Working directory: $WORKDIR"

# host requirements
for cmd in curl unzip tar sed awk grep base64 xxd tr find; do need "$cmd"; done
...
# sources
ADOPTIUM_JRE_URL="https://api.adoptium.net/v3/binary/latest/11/ga/linux/${ADOPTIUM_ARCH}/jre/hotspot/normal/adoptium"
...
b64_to_base32() {
  local b64="$1" tmp
  tmp="$(mktemp)"
  # GNU coreutils: base64 -d ; (we also try -D for macOS compatibility if someone copies this script)
...
find_apk_loop() {
  ...
  while true; do
    ...
    if (( ${#CANDIDATES[@]} == 0 )); then
      ...
      if command -v xdg-open >/dev/null 2>&1; then xdg-open "$APKMIRROR_DL_URL" >/dev/null 2>&1 || true; fi
...
# Linux JRE layout: */bin/java
JAVA_BIN="$(dirname "$(find . -type f -name 'java' -executable 2>/dev/null | head -n1)")"
if [[ -z "${JAVA_BIN:-}" ]]; then
  # fallback (just in case)
  JAVA_BIN="$(find . -type d -path '*Contents/Home/bin' -maxdepth 5 2>/dev/null | head -n1)"
fi
...
On your PHONE:
  ...
  • Connect the phone to this Linux PC via USB.
...
ok "You may now update the Steam app to the latest version through the Google Play Store."
